...
select transactions.id, amount, name from transactions left join transactions_types on transactions.id = transactions_types.id;

select transactions.id as transacción, amount as monto, name as tipo_de_transacción from transactions right join transactions_types on transactions.id = transactions_types.id;
...
create view v_accounts_types as select account_no, client_id, name from accounts left join account_types on accounts.id = account_types.id;

create view v_transactions_detail as select transactions.id, amount, name from transactions left join transactions_types on transactions.id = transactions_types.id;

create view v_transations_types_detail as select transactions.id as transacción, amount as monto, name as tipo_de_transacción from transactions right join transactions_types on transactions.id = transactions_types.id;
